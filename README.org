...
Paw is a Chrome/Firefox extension for sending page context (url, title, selected body, surrounding note) to Emacs via org-protocol, with optional server-side features. It adds a floating action button, keyboard shortcut, single-click word capture, auto-highlighting powered by a local server, and a OneTab-style tab manager.
...
- *OneTab-style tab manager*: capture, organise, and send tabs to Emacs
...
* Tab Manager
The tab manager (~tabs.html~) is a full-page OneTab-style interface for captured tabs.
...
- OneTab-style tab manager with drag-and-drop, multi-select, send-to-Emacs, multi-format copy
